...
# Configuration
$config = @{
    JsonPath = Join-Path $PSScriptRoot "../site/data/learning-timeline.json"
...
# Verify paths exist
if (-not (Test-Path $config.JsonPath)) {
    Write-Error "Cloud resume learning-timeline.json not found at: $($config.JsonPath)"
    exit 1
}
...
Write-Host "`nSource: $($config.JsonPath)" -ForegroundColor Gray
